		...
		protected:
			...
			void DebugDraw() {
				if (children) {
					...
					Vector3 vertices[4]{
						//Vector3(position.x - size.x, -100, position.y + size.y),
						//Vector3(position.x + size.x, -100, position.y + size.y),
						//Vector3(position.x + size.x, -100, position.y - size.y),
						//Vector3(position.x - size.x, -100, position.y - size.y),
						Vector3(position.x - size.x, 10, position.y + size.y),
						Vector3(position.x + size.x, 10, position.y + size.y),
						Vector3(position.x + size.x, 10, position.y - size.y),
						Vector3(position.x - size.x, 10, position.y - size.y)
					};
					Debug::DrawLine(vertices[0], vertices[1], Vector4(1, 0, 0, 1), 0.1f);
					Debug::DrawLine(vertices[1], vertices[2], Vector4(1, 0, 0, 1), 0.1f);
					Debug::DrawLine(vertices[2], vertices[3], Vector4(1, 0, 0, 1), 0.1f);
					Debug::DrawLine(vertices[3], vertices[0], Vector4(1, 0, 0, 1), 0.1f);
					//Debug::DrawLine(vertices[4], vertices[5], Vector4(1, 0, 0, 1));
					//Debug::DrawLine(vertices[5], vertices[6], Vector4(1, 0, 0, 1));
					//Debug::DrawLine(vertices[6], vertices[7], Vector4(1, 0, 0, 1));
					//Debug::DrawLine(vertices[7], vertices[4], Vector4(1, 0, 0, 1));
					//Debug::DrawLine(vertices[0], vertices[4], Vector4(1, 0, 0, 1));
					//Debug::DrawLine(vertices[1], vertices[5], Vector4(1, 0, 0, 1));
					//Debug::DrawLine(vertices[2], vertices[6], Vector4(1, 0, 0, 1));
					//Debug::DrawLine(vertices[3], vertices[7], Vector4(1, 0, 0, 1));
				}
			...
			void OperateOnRayCastContents(Ray& r, QuadTreeFunc& func) {
				...
				if (children) {
					...
					if (!contents.empty() && 
							(CollisionDetection::RayBoxIntersection(r, Vector3(position.x, 0, position.y), Vector3(size.x, 1000, size.y), thisCollision) ||
							 CollisionDetection::AABBTest(r.GetPosition(), Vector3(position.x, 0, position.y), Vector3(1.0f, 1.0f,1.0f), Vector3(size.x, 1000.0f, size.y)))) {
						//std::cout << "tested";
						func(contents);
					}
					//std::cout << std::endl;
...
namespace NCL {
	...
	namespace CSC8503 {
		...
		public:
			...
			void OperateOnRayCastContents(Ray& r, typename QuadTreeNode<T>::QuadTreeFunc  func) {
